...
def _generate_action_items_section(
    ...
    # Group alerts by issue key - combine all alerts for same issue into one row
    # Sort by priority first so most severe alerts appear first in combined output
    action_items.sort(key=lambda x: x["priority"])

    grouped_items = {}  # issue_key -> {summary, assignee, alerts: [(type, detail, priority)]}
    for item in action_items:
        ...
        if key not in grouped_items:
            grouped_items[key] = {
                ...
                "alerts": [],
                "min_priority": item["priority"]  # for sorting groups
            }
        grouped_items[key]["alerts"].append((item["type"], item["detail"]))

    # Sort groups by most severe alert (lowest priority number)
    sorted_groups = sorted(grouped_items.values(), key=lambda x: x["min_priority"])
    ...
    if sorted_groups:
        ...
        for group in sorted_groups:
            # Combine all types and details with <br> for multi-line in table cell
            types_combined = "<br>".join(alert[0] for alert in group["alerts"])
            details_combined = "<br>".join(alert[1] for alert in group["alerts"])
            lines.append(
                f"| {group['issue']} | {group['summary']} | {types_combined} | {details_combined} | {group['assignee']} |"
            )
        lines.append("")
    else:
    ...
    # Status summary for clarity (only show if no action items)
