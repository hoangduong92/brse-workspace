...
Common Workflows:
  1. Morning routine:  /bk-morning
  2. Check progress:   /bk-track status --lang ja
  3. Weekly report:    /bk-track report --format pptx
  4. Parse email:      /bk-capture task "email content..."
  5. Search context:   /bk-recall search "keyword"
""")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  bk-init                    Run interactive wizard (credentials + project config)
  bk-init --env-only         Only setup .env file (skip project config)
  ...
  bk-init --check-env        Check environment variables
    ...
    parser.add_argument(
        ...
        type=str,
        default=".",
        help="Output directory for project-context.yaml and .env (default: current)"
    )
    parser.add_argument(
        "--validate",
        action="store_true",
    ...
    print(f"  project-context.yaml: {saved_path}")
