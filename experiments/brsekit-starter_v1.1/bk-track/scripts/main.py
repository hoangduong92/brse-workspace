...
def cmd_status(args):
    ...
    project_id = os.getenv("NULAB_PROJECT_ID") or args.project

    if not space_url or not api_key or not project_id:
        print("Error: Missing NULAB_* environment variables")
        print("Set in .claude/skills/brsekit/.env:")
    ...
    try:
        ...
        # Read time logs from vault for report
        if TimeLogStore:
            try:
...
def main():
    ...
    parser.add_argument("--project", "-p", help="Backlog project key")
